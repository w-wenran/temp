...
public class ConsistencyHash {
    ...
    public void init() {
        ...
        for(int i=0; i<shards.size(); i++) {
            ...
            for(int j=0; j<VIRTUAL_NUM; j++) {
                byte[] md5 = computeMd5("SHARD-" + i + "-NODE-" + j);
                long hashValue = hash(md5,j);
	 ...
	 */
     ...
     public static void main(String[] args) {
         ...
         for(int i=0; i<50; i++) {
             long hashvalue = hash.hash(hash.computeMd5(String.valueOf(i)),ran.nextInt(hash.VIRTUAL_NUM));
             System.out.println(String.format("[%s-%s] store server %s ",i,hashvalue,hash.getShardInfo(hashvalue)));
         }
   }
